...
class Empty extends TweetSet {
  ...
  override def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = acc //直接返回到现在为止累积的树

  override def union(that: TweetSet): TweetSet = that //直接返回that
...
//    else left.filterAcc(p, right.filterAcc(p, acc))
  ...
  def compareTo(a : Tweet, b : Tweet): Tweet = {
  ...
  override def mostRetweeted : Tweet = {
    if(left.isEmpty && right.isEmpty){
      ...
      compareTo(elem, right.mostRetweeted)
    }
    else if (!left.isEmpty && right.isEmpty){
      compareTo(elem, left.mostRetweeted)
    }else {
      compareTo(compareTo(left.mostRetweeted, right.mostRetweeted), elem)
    }
  }
